...
int main() {
	...
	UINT_PTR pNtCreateFileAddr = NULL;
	pNtCreateFileAddr = (UINT_PTR)GetProcAddress(ntDll, "NtCreateFile");
	NtCreateFileSysCall = pNtCreateFileAddr + 0x12;

	RtlInitUnicodeString(&fileName, L"\\??\\C:\\ProgramData\\TEMP\\syscall.log");

	InitializeObjectAttributes(&objAttr, &fileName, OBJ_CASE_INSENSITIVE, NULL, NULL);

	status = MyNtCreateFile(&hFile,
	...
	if (status == 0)
		printf("[+] Second File was created successfully.\n");
	else
